...
export default class SunburstD3 {
    ...
    _update(figure) {
        ...
        const transitionDuration =
            figure.transitionDuration || dflts.transitionDuration;
        ...
        const selectedRadius = node => [
            node.y ? self.figure.innerRadius : 0,
            self.radius,
        ];
        ...
        const skinny = node => {
            const dtheta =
                self.angularScale(node.x + node.dx) - self.angularScale(node.x);
