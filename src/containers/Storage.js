...
class Storage extends Component {
  ...
  async componentDidMount() {
    const statsData = await API.get('files', '/storage-stats', {});

    this.setState({ statsData });

    const data = await API.get('files', '/files?query=&randomSamples=200', {});
  ...
  computeTree = (rootNode, dataRows, maximumDepth) => {
    dataRows.forEach((d) => {
      ...
      tokens.forEach((t, depth) => {
        ...
        child.size = dataRows.filter((d) => d[2].includes(fullPath)).length;
    ...
    return (
      <ResponsiveContainer width='100%' height={600}>
        <ScatterChart
          ...
          <CartesianGrid />
          <XAxis type='number' dataKey='childrenCount' name='Total Nodes' allowDecimals={false}>
            <Label value='Total Nodes' offset={-10} position='insideBottom' />
          </XAxis>
          <YAxis
            type='number'
            dataKey='weight'
            ...
            <Label value='Total Size (%)' angle={-90} position='insideLeft' />
          </YAxis>
          <ZAxis type='number' dataKey='weight' name='Total Size' range={[0, 1000]} unit='%' />
          <Tooltip
            cursor={{
              strokeDasharray: '3 3',
            }}
  ...
  renderSingleStatData = (stat) => (
    <Grid key={stat.label} item xs={6} md={3} lg={2}>
      <Card>
        <CardContent>
          <Grid container>
            <Grid item xs={12}>
            ...
            <Grid
              ...
              {stat.query && (
                <Button size='small' color='primary' href={`search?query=${stat.query}`}>
                  <SearchIcon />
                  View
                </Button>
              )}
  ...
  renderStatsData = () => {
    ...
    return (
      <Grid item xs={12} spacing={2} container direction='row' alignItems='stretch'>
  ...
  render() {
    ...
    return (
      <Grid className={classes.chartContainer} container>
        ...
        {treeData === undefined && <CircularProgress className={classes.progress} />}
        {treeData && (
          <Fragment>
            <Grid item xs={12}>
              <FormControl className={classes.formControl}>
                <InputLabel>Maximum Depth</InputLabel>
                <Select value={this.state.maximumDepth} onChange={this.handleMaxDepthChange}>
