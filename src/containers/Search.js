...
class Search extends Component {
  ...
  openSnackbar = () => this.props.searchResult.errorMessage;
  ...
  handleDownloadFile = async (bucket, key) => {
    try {
      const url = await API.get('files', `/file-signed-url?bucket=${bucket}&key=${key}`, {});
  ...
  renderRowButtons = (bucket, key, popupState) => (
    <Menu {...bindMenu(popupState)}>
      ...
      {(key.endsWith('bam') || key.endsWith('vcf')) && (
        <MenuItem onClick={() => this.handleIGVLink(bucket, key)}>
          <ListItemIcon>
      ...
      {!key.endsWith('bam') && (
        <MenuItem color='primary' onClick={() => this.handleDownloadFile(bucket, key)}>
          <ListItemIcon>
            <GetAppIcon />
          </ListItemIcon>
    ...
    return (
      <TableRow key={rowIndex}>
        {row.map((col, colIndex) => {
          return isColVisible(headers[colIndex].key) && <TableCell key={colIndex}>{col}</TableCell>;
        })}
        <TableCell>
  ...
  renderTable = () => {
    ...
    //TODO refactor search API data structure
    ...
    const headerRow = [
      ...
      { key: 'illumina_id', sortable: false },
      ...
      <Paper>
        <Table size='small' aria-label='a dense table'>
          ...
          <TableBody>
            {loading && (
              <TableRow>
                <TableCell colSpan={headerRow.length} style={{ textAlign: 'center' }}>
                  <CircularProgress />
                </TableCell>
              </TableRow>
            )}
